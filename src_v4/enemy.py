...
class Enemy(Sprite):
    ...
    def update(self, bullets, target_hero, t_interval, sound):
        ...
        if self.bullet_type != 3:
            ...
            self.fireTime += t_interval
            if self.fireTime > 0.03:
                self.fireTime = 0.0
                # 随机发射子弹，1/fire_T概率发射
        ...
        # boss
            ...
            if not self.appeared:
                ...
                self.fire_bullet(bullets, self.screen, self.shooting, t_interval, target_hero)
...
class Boss(Enemy):
    ...
    def fire_bullet(self, bullets, screen, typex, t_interval, target_hero=0):
        if typex == 0:
            tmp = self.shoot_dir0
