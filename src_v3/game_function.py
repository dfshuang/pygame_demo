...
def handle_events(sett, screen, hero, enemies, boss, bullets, bursts, stats, sb, sound, menu):
    """处理键盘和鼠标事件"""
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            handle_mouse(mouse_x, mouse_y, menu, stats, enemies, bullets)
...
def handle_keydown(sett, screen, event, enemies, boss, hero, bullets, bursts, stats, sb, sound):
    ...
    # 开始游戏
        ...
        if stats.isStartMenu:
            ...
            game_play(stats, enemies, bullets)
    # 没有开敌机
    elif not hero.plane:
        # 移动
        if event.key == pg.K_d:
...
def handle_mouse(mouse_x, mouse_y, menu, stats, enemies, bullets):
    """处理鼠标事件，刚开始的开始菜单"""
...
def game_play(stats, enemies, bullets):
    """开始游戏"""
    # 点击开始游戏
    if stats.start_game:
        # 重置游戏统计信息
